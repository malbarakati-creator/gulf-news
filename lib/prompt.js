...
// Builds the per-run user message. Pins "today" so the model dates items correctly.
export function buildUserMessage(now = new Date()) {
  const today = now.toISOString().slice(0, 10);
  return `Run today's Gulf morning sweep. Today's date is ${today}. Search live now across the listed sources (including the Arabic market sources and the culture/sport pass), apply the lens and recency rule, cross-check against the public Semafor Gulf site, and return ONLY the JSON object in the exact schema. Aim for 12+ well-dated items across the tiers.`;
}
